...
router.post('/',
    [auth,
        [
            check("targetAccountNumber", "Account number should be at least 8 carachters ! ")
                .isLength({ min: 8 }),
    ...
    , async (req, res) => {
        ...
        let targetAccount = await Profile.findOne({ accountNumber: targetAccountNumber });
        if (!targetAccount) { return res.status(404).json({ msg: 'Account not found' }); }

        let currentAccount = await Profile.findOne({ user: req.user.id });
        const { accountNumber, Balance } = currentAccount;

        if (accountNumber === targetAccountNumber) { return res.status(404).json({ msg: `you can't transfer to your account` }); }
        if (Balance - amount < 0) { return res.status(404).json({ msg: 'Balance is not enought' }); }
